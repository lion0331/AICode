...
{
    internal sealed class RefactorCodeCommand : BaseCommand
    {
        private new static readonly Guid CommandSet = new Guid("1A2B3C4D-5E6F-4A8B-9C0D-1E2F3A4B5C6D");
        private new const int CommandId = 0x0103;

        private RefactorCodeCommand(AsyncPackage package) : base(package, CommandSet, CommandId)
        {
        }
        ...
        public static async Task InitializeAsync(AsyncPackage package)
        {
            ...
            var command = new RefactorCodeCommand(package);
